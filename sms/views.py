    ...
    if request.method == 'POST':
        ...
        if type(search_key)==str:
            ...
            try:
                ...
                payments = WaterPaymentReceived.objects.raw('SELECT * FROM sms_waterpaymentreceived where account_number= %s or dest_msisdn=%s order by id desc', [search_key, search_key])
                ...
                bills_sent = WaterBillSent.objects.raw('SELECT * FROM sms_waterbillsent where account_number= %s or dest_msisdn=%s order by id desc', [search_key, search_key])
...
def meter_readings_report(request):
    ...
    if request.method == 'POST':
        ...
        if report_value=='4':
            ...
            for cust in customer:
                #summary_sheet.append((cust.id, cust.names, cust.last_meter_reading))
                summary_sheet.append((cust.pay_date, cust.account_number, cust.account_name,int(cust.amount),cust.received_from,cust.confirmation_code ,cust.balance_carried_forward,cust.client.network))
                number += 1
...
def water_court_allocations(request):
    ...
    if request.method == 'POST':
        ...
        context = {
            'courts': WaterCourt.objects.filter().order_by('name'),
            'courts_allocated': WaterCourtsReallocate.objects.filter().order_by('-id'),
            'clients': WaterClientAll.objects.filter().order_by('names')
        }
